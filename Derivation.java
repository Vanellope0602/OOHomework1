...
public class Derivation {
    ...
    public static void main(String[] args) {
        ...
        Pattern v = Pattern.compile("(\\d+\\s+\\d+)|(\\d+x)|([+-]\\s*[+-]\\s+\\d+)" +
                            "|([+-]{3,})|(\\d+\\*\\d+)|([^0-9x[+-](\\*)(\\^)\\s\\t])");
        ...
        while (m.find()) {
            ...
            if (deriItems[i].getDeriCoeff().equals(BigInteger.ZERO)) { //系数为0
                i++;
                continue; // ignore
            }
            ...
            for (int j = 0; j < i; j++) { // j 从0开始一个个比较是否出现过当前幂次数
                if (deriItems[j].getDeriExpo().equals(temp)) {
                    deriItems[j].fixCoeff(deriItems[i].getDeriCoeff()); //这样可以改变private 变量的值...
                    merge = true;
                    break;
                }
            }
        ...
        for (int k = 0; k < i; k++) {
            ...
            if (co.equals(BigInteger.ZERO)) { //若系数为0，则直接跳过
                ...
                if (k == 0 || first) { //第一项被输出的系数 不用输出加号
                    if (co.equals(BigInteger.ZERO.subtract(BigInteger.ONE))) { //系数为-1
                ...
                if (!ex.equals(BigInteger.ONE)) {//次数不等于1
                    if (co.equals(BigInteger.ONE) && !ex.equals(BigInteger.ZERO)) { //若系数为1，且次数不为0
    ...
    public void Get(String string) { //拿已经分割好的一个项进来提取，确保合法？
        ...
        for (int i = 0; i < temp.length; ) {
            ...
            if (!haveCoe && xoccur) { //出现了x,没写系数，但是有可能有符号，为1或-1
                ...
                //coeff = (isNeg) ? BigInteger.ZERO : BigInteger.ONE;
            }
            if (!haveExp && xoccur) {
